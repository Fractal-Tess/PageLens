<script lang="ts">
  ...
  import type { SiteFilesReport as SiteFilesReportType } from '../../../stores/report.svelte'
  import {
    getSeverityVariant,
    getSeverityClass
  } from '../../../stores/report.svelte'

  interface Props {
    report: SiteFilesReportType
    class?: string
  }
...
<div class="space-y-6 {className}">
      ...
      {:else}
        <div class="space-y-3">
          {#each report.sitemaps as sitemap}
            <Collapsible>
              <CollapsibleTrigger
                class="flex w-full items-center gap-2 text-sm hover:bg-muted rounded p-2 -mx-2"
              >
                <ChevronRight
  ...
  {#if report.issues.length > 0}
    <Card>
      <CardHeader class="pb-3">
        <CardTitle class="text-sm">Issues ({report.issues.length})</CardTitle>
      </CardHeader>
      <CardContent>
        <div class="space-y-2">
          {#each report.issues as issue}
            <div class="flex items-start gap-2 text-sm">
              <Badge
                ...
                class={cn('text-xs shrink-0', getSeverityClass(issue.severity))}
              >
                {issue.severity}
              </Badge>
