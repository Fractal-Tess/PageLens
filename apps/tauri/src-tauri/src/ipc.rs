...
pub async fn analyze_url(
    ...
    let execute: std::result::Result<AnalysisResult, String> = async {
        ...
        let snapshot = page.snapshot(options).await.map_err(|e| e.to_string())?;
...
#[tauri::command]
#[specta::specta]
pub fn get_history_item(id: String, db: State<Mutex<Database>>) -> Result<AnalysisRun, String> {
    let db_lock = db.lock().map_err(|e| e.to_string())?;
    let repo = db_lock.analysis_repository();
...
#[tauri::command]
#[specta::specta]
pub fn delete_history_item(id: String, db: State<Mutex<Database>>) -> Result<(), String> {
    let db_lock = db.lock().map_err(|e| e.to_string())?;
    let repo = db_lock.analysis_repository();
...
#[tauri::command]
#[specta::specta]
pub fn export_history(path: String, db: State<Mutex<Database>>) -> Result<(), String> {
    let db_lock = db.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
#[specta::specta]
pub fn import_history(path: String, db: State<Mutex<Database>>) -> Result<u32, String> {
    let db_lock = db.lock().map_err(|e| e.to_string())?;
    let path = PathBuf::from(path);
